...
using echo_serv::SQUARE_RPCConf;
using json = nlohmann::json;

void get_conf_from_env(SQUARE_RPCConf* conf){
    if (const char* env_app_name = std::getenv("SQUARE_RPC_APP_NAME")) {
        conf->app_name = env_app_name;
    }
    if (const char* env_app_version = std::getenv("SQUARE_RPC_APP_VERSION")) {
        conf->app_version = env_app_version;
    }
    if (const char* env_address = std::getenv("SQUARE_RPC_ADDRESS")) {
        conf->address = env_address;
    }
    if (const char* env_server_cert_path = std::getenv("SQUARE_RPC_SERVER_CERT_PATH")) {
        conf->server_cert_path = env_server_cert_path;
    }
    if (const char* env_server_key_path = std::getenv("SQUARE_RPC_SERVER_KEY_PATH")) {
        conf->server_key_path = env_server_key_path;
    }
    if (const char* env_root_cert_path = std::getenv("SQUARE_RPC_ROOT_CERT_PATH")) {
        conf->root_cert_path = env_root_cert_path;
    }
    if (const char* env_client_certificate_request = std::getenv("SQUARE_RPC_CLIENT_CERT_REQUEST")) {
    ...
    if (const char* env_max_rec_msg_size = std::getenv("SQUARE_RPC_MAX_REC_MSG_SIZE")) {
        try {
    ...
    if (const char* env_max_send_msg_size = std::getenv("SQUARE_RPC_MAX_SEND_MSG_SIZE")) {
        try {
    ...
    if (const char* env_max_concurrent_streams = std::getenv("SQUARE_RPC_MAX_CONCURRENT_STREAMS")) {
        try {
    ...
    if (const char* env_max_connection_idle = std::getenv("SQUARE_RPC_MAX_CONNECTION_IDLE")) {
        try {
    ...
    if (const char* env_max_connection_age = std::getenv("SQUARE_RPC_MAX_CONNECTION_AGE")) {
        try {
    ...
    if (const char* env_max_connection_age_grace = std::getenv("SQUARE_RPC_MAX_CONNECTION_AGE_GRACE")) {
        try {
    ...
    if (const char* env_keepalive_time = std::getenv("SQUARE_RPC_KEEPALIVE_TIME")) {
        try {
    ...
    if (const char* env_keepalive_timeout = std::getenv("SQUARE_RPC_KEEPALIVE_TIMEOUT")) {
        try {
    ...
    if (const char* env_keepalive_enforement_permit_without_stream = std::getenv("SQUARE_RPC_KEEPALIVE_ENFORCEMENT_PERMIT_WITHOUT_STREAM")) {
    ...
    if (const char* env_log_level = std::getenv("SQUARE_RPC_LOG_LEVEL")) {
    ...
    if (const char* env_compression = std::getenv("SQUARE_RPC_COMPRESSION")) {
...
void get_conf_from_cmd(SQUARE_RPCConf* conf,int argc, char** argv){
    argparse::ArgumentParser program("echo","0.0.0");
